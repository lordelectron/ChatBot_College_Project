...
public class ChatBotQuestionsAndAnswers extends JFrame {
	private static final long serialVersionUID = 1L;

	public static void QuestionAnswers(String text, JTextArea area) {
		/*
		 ...
		 */
		if (text.contains("hi")) {
			replyMeth("Hi there", area);// calling the replymeth(...) method for the answers
		} else if (text.contains("how are you")) {
			replyMeth("I'm Good :).Thankyou for asking", area);
		} else if (text.contains("what is your name")) {
			replyMeth("I'm Mya", area);
		 ...
		 */
			...
			replyMeth("opening notepad ", area);
			...
			{
				replyMeth("File deleted successfully", area);
			} else {
				replyMeth("Failed to delete the file", area);
			}
			...
			try {
				replyMeth("Shutting down the PC after 5 seconds.", area);
		 ...
		 */
			...
			replyMeth("opening minecraft java edition", area);
			...
			if (flist == null) {
				replyMeth("Empty directory.", area);
			} else {
				for (int i = 0; i < flist.length; i++) {
					...
					if (filename.equalsIgnoreCase("abc.txt")) // checking if the searched file is identical
																// to an existing file
					{
						replyMeth(filename + " found at" + "\n" + " C:\\Users\\Soumyajit\\Desktop", area);
						flag = 1;
					}
				}
			}
	...
	public static void replyMeth(String reply, JTextArea area) {
